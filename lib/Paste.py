...
class Paste(object):
	...
	def match(self):
		...
		for regex in regexes['blacklist']:
			if regex.search(self.text): self.db_keywords -= 1.5 * (1/float(len(regexes['db_keywords'])))
		for regex in regexes['whitelist']:
			if regex.search(self.text): self.db_keywords = .9
